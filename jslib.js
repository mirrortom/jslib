...
factory.extend({
  ...
  /**
   * 从已经选择的元素中,筛选出符合条件的
   * @param {function} condition 条件函数,参数是dom元素,返回ture/false
   * @returns {jslib} 返回this
   */
  ...
  /**
   * 检查第一个匹配的元素是否含有指定的属性(原生: hasAttribute)
   * @param {string} val 属性名字
   * @returns {boolean} 第一个匹配含有属性名字时返回true,其它情况返回false
   */
  'hasProp': function (val) {
    if (this.length > 0) {
      return this[0].hasAttribute(val);
    }
    return false;
  },
  /**
...
((win) => {
  function _back() {
    ...
    win.onpopstate = function (event) {
      // location.hash=#xxx,这个url的hash属性值修改,也会触发这个事件,这是以前另一种实现回退的方式.
      // 要排除掉这种触发行为.
